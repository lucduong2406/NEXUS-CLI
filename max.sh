...
# Cải tiến: Đọc node_ids từ tệp node_ids.txt để dễ dàng thêm/xóa ID
...
NODE_IDS_FILE="node_ids.txt"  # Tệp chứa danh sách node IDs
...
# Kiểm tra và tạo tệp node_ids.txt nếu chưa tồn tại
if [ ! -f "$NODE_IDS_FILE" ]; then
    echo "Tệp $NODE_IDS_FILE không tồn tại. Tạo tệp mẫu..." | tee -a "$LOG_FILE"
    cat << EOF > "$NODE_IDS_FILE"
# Danh sách node IDs, mỗi dòng chứa một ID
# Ví dụ:
# 12485507
# 12485503
12485507
...
12259088
EOF
    echo "Đã tạo tệp mẫu $NODE_IDS_FILE. Vui lòng kiểm tra và chỉnh sửa nếu cần." | tee -a "$LOG_FILE"
fi
...
if [ ${#node_ids[@]} -eq 0 ]; then
    echo "Lỗi: Không tìm thấy node ID hợp lệ trong $NODE_IDS_FILE. Vui lòng thêm ít nhất một ID." | tee -a "$LOG_FILE"
    exit 1
fi
echo "Đã đọc ${#node_ids[@]} node ID từ $NODE_IDS_FILE." | tee -a "$LOG_FILE"
...
echo "Để chỉnh sửa node IDs, mở tệp $NODE_IDS_FILE và thêm/xóa ID (mỗi ID trên một dòng)." | tee -a "$LOG_FILE"
