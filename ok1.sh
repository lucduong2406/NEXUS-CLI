...
# Script tự động cài đặt tmux (nếu chưa có), Nexus CLI, và chạy nexus-network trong các cửa sổ tmux riêng biệt trên Ubuntu
...
# Kiểm tra và cài đặt tmux nếu chưa có
if command -v tmux &> /dev/null; then
    echo -e "${GREEN}tmux đã được cài đặt sẵn! Phiên bản:${NC}"
    tmux -V
else
...
echo -e "${GREEN}Đang tạo hoặc cập nhật tệp cấu hình .tmux.conf...${NC}"
...
if command -v curl &> /dev/null; then
    echo -e "${GREEN}curl đã được cài đặt sẵn!${NC}"
else
    ...
    echo -e "${GREEN}curl đã được cài đặt thành công!${NC}"
fi
...
echo -e "${GREEN}Vui lòng nhập URL chứa danh sách node ID (nhấn Enter để sử dụng URL mặc định: https://raw.githubusercontent.com/lucduong2406/NEXUS-CLI/main/12):${NC}"
read -r NODE_URL
if [ -z "$NODE_URL" ]; then
